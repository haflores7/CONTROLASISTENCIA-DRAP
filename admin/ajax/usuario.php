...
switch ($_GET["op"]) {
	...
	case 'verificar':
		...
		error_log("Intento de login - Usuario: $logina");
		
		$rspta = $usuario->verificar($logina, $clavehash);
		$fetch = $rspta->fetch_object();

		if (isset($fetch)) {
			error_log("Usuario encontrado: " . $fetch->nombre);
			...
			error_log("Sesión iniciada - Variables de sesión establecidas");
			
			$response = [
				'status' => 'success',
				'usuario' => [
					'nombre' => $fetch->nombre,
					'tipousuario' => $fetch->tipousuario
				]
			];
		} else {
...
error_log('Sesión iniciada para usuario: ' . $_SESSION['nombre']);
?>